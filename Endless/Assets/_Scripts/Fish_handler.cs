...
public class Fish_handler : MonoBehaviour
{
    ...
    public float tankSphereDistance;
    public float cornerScale = 0.5f;

    void Awake()
    {
        tank = GameObject.Find("tank");
        tankCenter = tank.transform.position;
    ...
    void MoveFollow()
    {
        ...
        if (distance > ZONE_TWO_DIST && distance < ZONE_THREE_DIST)
        {//need to get closer
        ...
        else if (distance > ZONE_ONE_DIST && distance < ZONE_TWO_DIST)
        {//get into position
    ...
    void Update()
    {
        ...
        if (!rb.useGravity)
        {//if the fish is alive and well
            if (data.isSchoolLeader || leaderFish == null)
            {//if the fish belongs to noschool or is the leader of its school
                MoveRandom();
            }
            else
            {//if the fish is a follower of it's school
                MoveFollow();
            }
            transform.Rotate(0, 0, -transform.rotation.eulerAngles.z);
        }
        else
        {//dead fish rolls onto it's side
            MoveDead();
        }
